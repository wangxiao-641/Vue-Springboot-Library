...
以下三个业务接口由后端统一完成校验、日期计算、库存更新和记录更新。前端只需发送读者和图书标识。
...
**失败响应示例：**
```json
{"code": "-1", "msg": "该图书已全部借出，暂无可借副本"}
{"code": "-1", "msg": "您已借阅该书，不可重复借阅"}
{"code": "-1", "msg": "您已达到最大借阅数量(5本)"}
{"code": "-1", "msg": "您有逾期未还图书，请先归还"}
```
...
**失败响应示例：**
```json
{"code": "-1", "msg": "您未借阅该书，无法归还"}
```

### 12.3 续借

续借规则：最多 1 次，每次延长 30 天。
